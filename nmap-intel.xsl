  ...
  "hosts": [<xsl:for-each select="/nmaprun/host">
    {
      ...
      "ports": [<xsl:for-each select="ports/port">{"port":<xsl:value-of select="@portid"/>,"proto":"<xsl:value-of select="@protocol"/>","state":"<xsl:value-of select="state/@state"/>","svc":"<xsl:value-of select="service/@name"/>","product":"<xsl:call-template name="escape-json"><xsl:with-param name="text" select="service/@product"/></xsl:call-template>","version":"<xsl:call-template name="escape-json"><xsl:with-param name="text" select="service/@version"/></xsl:call-template>","cpe":"<xsl:value-of select="service/cpe"/>","fp":"<xsl:call-template name="escape-json"><xsl:with-param name="text" select="service/@servicefp"/></xsl:call-template>","scripts":[<xsl:for-each select="script">{"id":"<xsl:value-of select="@id"/>","output":"<xsl:call-template name="escape-json"><xsl:with-param name="text" select="@output"/></xsl:call-template>"}<xsl:if test="position()!=last()">,</xsl:if></xsl:for-each>]}<xsl:if test="position()!=last()">,</xsl:if></xsl:for-each>],
